...
values. 3. Improvise (2) where in you will interchange the nodes themselves
*/
...
struct node *insert_node( struct node *start, struct node *insert, int node_no){
...
int main(){
	struct node *first,*tem,*bk, *new, *new2;
	int val=1;
	int node1, node2;
	...
	while(v){
		first=bk;
	printf("There are 8 nodes in the linked list, Enter 2 nodes to interchange nodes. Enter two 1-8, 0 to exit\n");
	scanf("%d", &node1);
	scanf("%d", &node2);
	
	if((node1<9 && node1>0)&&(node2<9 && node2>0) && (node1!=node2)){
	if(node1>node2){
		node1=node1+node2;
		node2=node1-node2;
		node1=node1-node2;
	...
	new=go_to_node(bk,node1-1);
	new2=go_to_node(bk, node2-1);
	if(node1!=1){
	first->next=go_to_node(bk,node1);
	}else{
	bk=first->next;
	}
	new=insert_node(bk,new,node2);


	first=bk;
	for(int i=1;i<node2-2;i++){
		first=first->next;
		}

	if(node2!=2){
	first->next=go_to_node(bk,node2-1);
	}else{
	bk=first->next;
	}
	new2=insert_node(bk,new2,node1);
	if(node1==1){
	bk=new2;
		}
	}
